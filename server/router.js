...
app.post('/api/registerUser', (req, response) => {
    MongoClient.connect(url, {useNewUrlParser: true}, function (err, db) {
        ...
        const {query: {username, password, telephone}} = req;
...
app.post("/api/login", function (req, response) {
    MongoClient.connect(url, {useNewUrlParser: true}, function (err, db) {
        ...
        dbo.collection('userInfo').findOne({username: username, password: base64.encode(password)}, function (err, res) {
            if (err) throw err;
            if(!!res) {
                req.session.userInfo = {username: res.username, telephone: res.telephone};
                response.status(200).json({code: '200', message: '登录成功', info: {username: res.username, telephone: res.telephone}});
            } else {
...
app.get("/api/check", function (req, res) {
    if(!!req.session.userInfo){
        res.status(200).json({code: '200', message: `欢迎回来${req.session.userInfo.username}`})
    } else {
